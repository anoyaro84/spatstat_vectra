...
for (sample_name in paths[1:1]){
  ...
  print(paste("dimensions of the data is", dim(data)[1], "times", dim(data)[2])) # 1805834 197
  data_filtered = data %>% filter(Phenotype != "")
  print(paste("dimensions of the data is", dim(data_filtered)[1], "times", dim(data_filtered)[2])) # 1805653 197
  
  data_filtered_with_distance <- data_filtered %>%
    do(bind_cols(., find_nearest_distance(.)))
  print(paste("dimensions of the data is", dim(data_filtered_with_distance)[1], "times", dim(data_filtered_with_distance)[2])) #1805653 217
  
  pairwise_distances = distance_matrix(data_filtered_with_distance)
  output =  do_analyse(data_filtered_with_distance, pheno_vector_absolut, colors_absolut,NULL, plotter = c(TRUE,TRUE),XposCol = 'Cell X Position', YposCol = 'Cell Y Position',PhenoCol = 'Phenotype', sample_name, r_vec = NULL)
  break
