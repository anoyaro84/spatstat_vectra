...
selected_spatstat_statistics = 'ALL' # 'ALL', NULL, list('Gdot', 'K')

# debug

# extreme = file.path(base_path,'HO105-061_[11807,41972]_cell_seg_data.txt')

extreme = c(extreme[3],D[8])

# extreme = file.path(base_path, paste0(c(sixes_names, threes_names),'_cell_seg_data.txt'))
...
border_images_data_unfil = HO105_image_check$`Border images (% border)`
...
feature_matrix_simple = readRDS(path.expand('~/Studie/Thesis - Local/Extracted_Features_22ndJun.RDS'))

cols_without_J = str_subset(colnames(feature_matrix_simple),'J_radius', negate = TRUE)
cols_without_Jdot = str_subset(colnames(feature_matrix_simple),'Jdot_radius', negate = TRUE)
cols_without_J_Total = intersect(cols_without_J, cols_without_Jdot)
feature_matrix_simple_without_J = feature_matrix_simple[,cols_without_J_Total]

# View(rowSums((is.na(feature_matrix_simple_without_J))))
# View(colSums((is.na(feature_matrix_simple_without_J)))) # 22 = 14 + 8 times MED_min_Tcells_Tcells == NA
# View(colSums((feature_matrix_simple_without_J ==0))) # 14 times 0 Tcells
# View(colSums((feature_matrix_simple_without_J ==1))) # 8 times 1 Tcell

NAs_per_sample = rowSums((is.na(feature_matrix_simple_without_J)))
# View(feature_matrix_simple_without_J[rowSums((is.na(feature_matrix_simple_without_J)))>0,])
df_and_NAs = cbind(NAs_per_sample,feature_matrix_simple_without_J)
View(df_and_NAs)
