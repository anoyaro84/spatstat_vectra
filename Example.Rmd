...
#     component_path = component_path = sub("_cell_seg_data.txt", "_component_data.tif", file)
#     info = get_field_info(component_path)
#     pixels_per_micron_file = 1/info$microns_per_pixel
#	  data_getOption = purrr::map_df(file, read_cell_seg_data, pixels_per_micron = getOption("phenoptr.pixels.per.micron"),remove_units = FALSE)
    data_auto = purrr::map_df(file, read_cell_seg_data, pixels_per_micron = "auto",remove_units = FALSE)
	
	
  	samplename = gsub('_cell_seg_data.txt', '', tail(strsplit(file, '/')[[1]],1))
	outputs_auto[[samplename]] = do_analyse(Intable = data_auto, PhenoOrder = phenotype_simple, ColsOrder = color_simple,
...
M_auto = feature_extract(outputs_auto, fig.prefix.simple)
```
...
base_path <- '../Exported data tables/' # Working for Yongsoo
base_path <- 'C:/Users/erikb/Documents/Thesis - Local/Data Marit voor Erik Bosch/Erik Bosch/Data complete phenotyping/' # working for Erik
# exported_data_path = 'HO105_Exported data tables/'
# exported_data_full_path = paste0(base_path, exported_data_path)
...
#### renaming segmentation and component files in directory with '_2_', '_3_' and '_4_' (replace with '_') ####
all_files_x_ = list.files(base_path, pattern =  'HO105-',full.names = TRUE)
new_names = str_replace_all(all_files_x_, pattern = '\\_1\\_|\\_2\\_|\\_3\\_|\\_4\\_|\\_5\\_|\\_6\\_', replacement = '\\_')
...
for (file in pathcombi){
	...
	data = purrr::map_df(file, read_cell_seg_data, pixels_per_micron = 'auto', remove_units = FALSE)
