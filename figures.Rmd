...
FX = alltypes(X, 'Fest')
FX$title =  'The Empty space function with reduced sample border correction\n for Marked Poisson Point Process X'

# png(filename = 'fig_example_emptyspace_X.png', width = 1440, height = 1440)
# layout(matrix(c(1,2,3,3), 2, 2, byrow = TRUE))
# # par(oma=c(0,0,0,0), mar = c(2,0,3,1)+0.1)
# plot(X1, main = NULL, cols = c("blue", "red")[1], pch = 20)
# title(sub = TeX('50 points of phenotype 1'), line=0.5)
# plot(X2, main = NULL, cols = c("blue", "red")[2], pch = 20)
# title(sub = TeX('150 point of phenotype 2'), line=0.5)
# # par(oma=c(0,0,0,0),mar = c(5,0,1,1)+0.1)
# plot(FX,cbind(rs, theo) ~ r, xlim = c(0,0.275), samex = TRUE, legend = TRUE)
# dev.off()


png(filename = 'fig_example_emptyspace_X.png', width = 720, height = 480)
par(oma=c(0,0,0,0), mfrow = c(1,2), mar = c(1,0,1,1)+0.1)
plot(X1, main = NULL, cols = c("blue", "red")[1], pch = 20)
title(sub = TeX('50 points of phenotype 1'), line = -1.5)
plot(X2, main = NULL, cols = c("blue", "red")[2], pch = 20)
title(sub = TeX('150 point of phenotype 2'), line = -1.5)
mtext(TeX('Marked Point Process X with 200 points in the unit square observation window'), side=3, cex = 1, line=-2.5, outer=TRUE)
dev.off()

png(filename = 'fig_example_emptyspace_X_func.png', width = 720, height = 480)
par(oma=c(0,0,0,0),mar = c(5,0,1,1)+0.1)
plot(FX,cbind(rs, theo) ~ r, xlim = c(0,0.275), samex = TRUE, legend = TRUE)
dev.off()

FXE = alltypes(X, 'Fest',envelope = TRUE)

FXE$title =  'The Empty space function with reduced sample border correction and significance band \n for Marked Poisson Point Process X'

png(filename = 'fig_example_emptyspace_X_func_env.png', width = 720, height = 480)
par(oma=c(0,0,0,0),mar = c(5,0,1,1)+0.1)
plot(FXE, xlim = c(0,0.275), samex = TRUE, legend = TRUE)
# plot(FX,cbind(rs, theo) ~ r, xlim = c(0,0.275), samex = TRUE, legend = TRUE)
# plot(envelope(X),add = TRUE)
dev.off()
...
for (i in seq_along(Xsplit)){
  allX = alltypes(Xsplit[[i]],fun = 'Fest', correction = 'km',envelope = TRUE)
  envX = envelope(Xsplit[[i]],fun = 'Fest', correction = 'km')
  plot(alltypes(Xsplit[[i]],fun = 'Fest', correction = 'km',envelope = TRUE))
  plot(envelope(Xsplit[[i]],fun = 'Fest', correction = 'km'))
}

# plot(alltypes(X,'Fest', correction = 'km', envelope = TRUE)) BAD



# for (i in seq_along(Xsplit)){
#     plot(alltypes(Xsplit[[i]],'Fest',envelope = TRUE),same)
# }
